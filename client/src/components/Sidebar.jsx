      ...
      <div className='flex flex-col'>
        {userDummyData.map((user, index) => (
          <div
            ...
            key={index}
            className={`relative flex items-center gap-2 py-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?.id === user._id && 'bg-[#282142]/50'}`}
          >
            <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-[35px] aspect-[1/1] rounded-full' />
            <div className='flex flex-col leading-5'>
              {/* ✅ FIX: Changed fullname → name */}
              <p>{user.fullName}</p>
              {index < 3
